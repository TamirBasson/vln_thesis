...
class RobotChatGUI(QWidget):
    ...
    def init_ui(self):
        ...
        context_label = QLabel("Environment:")
        context_label.setStyleSheet("font-weight: bold; font-size: 12px;")
        self.context_input = QLineEdit()
        self.context_input.setPlaceholderText("e.g., Warehouse, Home, Office...")
        self.context_input.setText("Home")  # Default value
        context_button = QPushButton("Set Context")
        context_button.clicked.connect(self.on_set_context)
...
def main(args=None):
    ...
    # Send initial environment context
    import time
    time.sleep(0.5)  # Wait for connections
    ros_node.send_context("Home")
